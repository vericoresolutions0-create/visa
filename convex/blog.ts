...
// ─── Seed data ────────────────────────────────────────────────────────────────
// The 10 original articles, preserved exactly as authored. Stored here so the
// admin can load them once via adminSeedArticles and then edit/add/remove from
// the admin panel without any further code changes or deploys.
...
The good news: all seven of these are fixable before you submit. A proper preparation process eliminates the vast majority of preventable rejections.`,
  ...
  {
    slug: "schengen-visa-guide-nigerians",
...
Immigration rules in Poland — particularly around work declarations and permit categories — change frequently. Always verify current requirements with the Polish embassy in your country before beginning the process. VisaClear's checklist for Poland is updated regularly.`,
  ...
  {
    slug: "travel-insurance-visa-applications",
...
Our Agents Marketplace connects you with verified agents for cases that genuinely warrant professional help.`,
  ...
  {
    slug: "canada-visitor-visa-africa",
...
Apply **4–5 months** before your intended travel date.`,
  ...
  {
    slug: "how-to-write-cover-letter-visa",
...
A cover letter does not save a weak application. If your financial proof is inadequate or your ties to home country are thin, no letter will fix that. The letter works best when your core documents are already strong — it adds context and removes doubt.`,
  ...
  {
    slug: "uae-visa-from-nigeria-ghana",
...
export const adminUpsert = mutation({
  args: {
    _id: v.optional(v.id("blog_articles")),
    slug: v.string(),
    title: v.string(),
    excerpt: v.string(),
    body: v.string(),
  ...
  handler: async (ctx, { _id, ...fields }) => {
    ...
    const slug = fields.slug.trim().toLowerCase().replace(/\s+/g, "-");

    // Check slug uniqueness (exclude current doc when updating)
    const existing = await ctx.db
      .query("blog_articles")
      .withIndex("by_slug", (q) => q.eq("slug", slug))
      .unique();
    if (existing && existing._id !== _id) {
      throw new ConvexError({ code: "CONFLICT", message: "An article with this slug already exists." });
    }

    if (_id) {
      await ctx.db.patch(_id, { ...fields, slug });
      return _id;
    }

    return await ctx.db.insert("blog_articles", {
      ...fields,
      slug,
...
export const adminTogglePublished = mutation({
  args: { _id: v.id("blog_articles") },
  handler: async (ctx, { _id }) => {
    await requireAdmin(ctx);
    const article = await ctx.db.get(_id);
...
export const adminSeedArticles = mutation({
  args: {},
  handler: async (ctx) => {
    await requireAdmin(ctx);
    const user = await getUserOrThrow(ctx);
    ...
    for (const article of SEED_ARTICLES) {
      await ctx.db.insert("blog_articles", {
        ...article,
        published: true,
        createdByUserId: user._id,
      });
