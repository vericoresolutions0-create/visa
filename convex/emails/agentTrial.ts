...
// ─── Agent trial expiring warning email ───────────────────────────────────────
export const sendAgentTrialExpiringEmail = internalAction({
  args: {
    to: v.string(),
    name: v.string(),
    plan: v.union(
  ...
  handler: async (ctx, args) => {
    ...
    const formattedDate = new Date(args.expiresAt).toLocaleDateString(
      "en-GB",
    ...
    const urgency = args.daysRemaining <= 1 ? "URGENT — " : "";
    const subject = `${urgency}Your ${planLabel} trial ends in ${args.daysRemaining} ${dayWord} — VisaClear`;
    const badgeColor = args.daysRemaining <= 1 ? "#dc2626" : "#d97706";
    const badgeBg = args.daysRemaining <= 1 ? "#fef2f2" : "#fffbeb";
    const badgeBorder = args.daysRemaining <= 1 ? "#fca5a5" : "#fcd34d";
        ...
        <tr>
          <td style="padding:36px 40px 28px;">
            <div style="display:inline-block;background:${badgeBg};border:1px solid ${badgeBorder};border-radius:8px;padding:8px 16px;font-size:12px;font-weight:600;color:${badgeColor};margin-bottom:20px;">
              TRIAL ENDS IN ${args.daysRemaining} ${dayWord.toUpperCase()}
            </div>
            <h2 style="font-family:Georgia,serif;font-size:22px;color:#0f2040;margin:0 0 8px;font-weight:600;">Your Free Trial Is Ending Soon</h2>
            <p style="font-size:14px;color:#555;line-height:1.7;margin:0 0 4px;">Hi ${escapeHtml(args.name)},</p>
            <p style="font-size:14px;color:#555;line-height:1.7;margin:0 0 20px;">
              Your free trial of the <strong>${escapeHtml(planLabel)}</strong> plan ends soon. Subscribe now to keep your marketplace placement, client pipeline, and listing visibility without any interruption.
            </p>
            <div style="background:#f8f5ef;border-radius:10px;padding:18px 20px;margin-bottom:24px;border-left:3px solid #b8953c;">
              <div style="font-size:12px;color:#b8953c;font-weight:600;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px;">Plan</div>
              <div style="font-size:16px;color:#0f2040;font-weight:600;">${escapeHtml(planLabel)} — Trial</div>
              <div style="font-size:13px;color:#666;margin-top:4px;">Ends: ${formattedDate}</div>
            </div>
