    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/30" onClick={onClose}>
      <div className="bg-background w-full max-w-md h-full overflow-y-auto p-6 space-y-6" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <h4 className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Business Details</h4>
          <Input placeholder="Department" value={department} onChange={(e) => setDepartment(e.target.value)} />
          <Input placeholder="Role / Title" value={roleTitle} onChange={(e) => setRoleTitle(e.target.value)} />
          <Input type="date" placeholder="Target relocation date" value={targetDate} onChange={(e) => setTargetDate(e.target.value)} />
        ...
        <div className="space-y-3">
          <h4 className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Private Notes</h4>
          <p className="text-xs text-muted-foreground">Only visible to your organisation — never shown to the employee.</p>
          <div className="space-y-2">
            <Textarea placeholder="Add a note…" value={noteText} onChange={(e) => setNoteText(e.target.value)} rows={3} />
            <Button size="sm" onClick={() => { void handleAddNote(); }} className="cursor-pointer font-semibold">Add Note</Button>
          </div>
          <div className="space-y-2">
            {notes === undefined ? (
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
      {PIPELINE_COLUMNS.map((column, idx) => {
        ...
        const nextStage = PIPELINE_COLUMNS[idx + 1]?.stage;
        return (
          <div key={column.stage} className="space-y-2.5">
            ...
            {colRows.map((row) => (
              <article key={row.linkId} className="rounded-lg border border-border bg-card p-3 shadow-sm space-y-2">
                ...
                {nextStage && (
                  ...
                  >
                    Move to {PIPELINE_COLUMNS[idx + 1].label} <ChevronRight className="w-3 h-3" />
                  </button>
                )}
              </article>
            ))}
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-serif text-3xl font-semibold text-primary">{myOrg.name}</h1>
          <p className="text-sm text-muted-foreground">{cohort.length} {cohort.length === 1 ? "person" : "people"} in your cohort</p>
        </div>
        <div className="flex gap-2">
          <Button
            variant="outline"
          ...
          >
            <Download className="w-4 h-4 mr-1.5" /> Export Compliance Report
          </Button>
        </div>
      </div>
        ...
        {view === "table" && (
          <>
            <div className="relative">
              ...
              <Input placeholder="Search name, email, department…" value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8 h-9 w-64" />
            </div>
            <select
              value={statusFilter}
      ...
      {cohort.length === 0 ? (
        <div className="border border-dashed border-border rounded-xl p-12 text-center text-sm text-muted-foreground">
          Invite your first employee to see your cohort build up here.
        </div>
      ) : view === "pipeline" ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/30 text-xs text-muted-foreground uppercase tracking-wide">
              <tr>
                ...
                <th className="text-left px-4 py-2.5">Readiness</th>
            ...
            <tbody className="divide-y divide-border">
              {filtered.map((row) => (
                <tr key={row.linkId} className="hover:bg-muted/20 cursor-pointer" onClick={() => setSelectedRow(row)}>
                  ...
                  <td className="px-4 py-3 text-muted-foreground flex items-center gap-1"><ClipboardList className="w-3.5 h-3.5" />{row.noteCount}</td>
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="border-b border-border/40 px-6 py-4 flex items-center gap-3">
        ...
        <span className="text-xs text-muted-foreground tracking-widest uppercase flex items-center gap-1"><Building2 className="w-3 h-3" /> for Employers</span>
      </header>
